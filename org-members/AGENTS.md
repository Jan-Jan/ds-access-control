# AGENTS.md (org-members)

Guidance specific to the `org-members` Rust library. See `../AGENTS.md` for
project-wide guidance.

## Crate context

Phase 1.a of the two-tier access-control project. Immutable binary Sparse
Merkle Tree (SMT) for organisation membership. `no_std`/WASM-compilable and
depends only on `alloc`. The local-first collaboration layer consumes this
lib's types (using the `p2p_key` as the member-as-a-group key when granting
access) but is out of scope here.

Design doc (canonical): `../docs/plans/2026-05-07-org-members-design.md` (in
main worktree, not always in feature worktrees).

## Crate-specific preferences
...
Tests live in:
- `tests/integration_test.rs` -- example/unit-style scenarios
- `tests/fuzz_tests.rs` -- proptest invariants (no panics, count
...
- `src/node.rs` -- internal Node type with `spin::Once` hash
- `src/hasher.rs` -- TrieHasher trait + Blake3Hasher default
