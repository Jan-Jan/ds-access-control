...
//! extension; not done yet because Task 7 tests are serial.
...
/// Hub" (signalling the WS metadata path is ready too).
///
/// Returns when the fork is ready to serve `chainHead_v1_*`. The caller
/// is responsible for any deploy step (the e2e test uses the existing
...
pub async fn spawn_fork() -> Result<ChopsticksHandle, SpawnError> {
    ...
    // Wait briefly for the port to be free.
    ...
    // the Drop impl can `killpg` the whole tree.
    unsafe {
        cmd.pre_exec(|| {
            if libc::setsid() == -1 {
...
/// Poll until the port is free or the budget elapses.
